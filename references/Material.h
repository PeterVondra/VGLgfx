...
namespace vgl
{
	...
	class Material
	{
		...
		private:
			...
			friend class vk::BaseRenderer;
			friend class vk::ForwardRenderer;
			friend class vk::DeferredRenderer;
	...
	// Uses Uber shader and sets specified macros for configuration when loading the uber shader
	class ShaderPermutationsGenerator
	{
		public:
			...
			static SHADER_DATA_T generateShader(ShaderInfo& p_ShaderInfo, ShaderType p_ShaderType);
			static SHADER_DATA_T loadShader(std::string p_VertexShaderPath, std::string p_FragmentShaderPath);
			inline static void setUberShaderPBR(SHADER_DATA_T& p_Shader) { m_Shader = p_Shader; }
	...
	inline std::vector<std::string> ShaderPermutationsGenerator::generateShader(ShaderInfo& p_ShaderInfo, ShaderType p_ShaderType)
	{
		...
		// PBR
		if (p_ShaderInfo.p_LightingType & (1 << 0) && p_ShaderType == ShaderType::Forward)
			ubershader = loadShader("resources/Shaders/UberPBR.vert.glsl", "resources/Shaders/UberPBR.frag.glsl");
		else if (p_ShaderInfo.p_LightingType & (1 << 0) && p_ShaderType == ShaderType::Deferred)
			ubershader = loadShader("resources/Shaders/UberPBRDeferred.vert.glsl", "resources/Shaders/UberPBRDeferred.frag.glsl");
...
=======
		...
		// Light caster = direction
		if (p_ShaderInfo.p_LightCaster & (uint32_t)LightingType::PBR)
			shader[1] += "#define LIGHT_DIR\n";
		// Light caster = point
		if (p_ShaderInfo.p_LightCaster & (uint32_t)LightCaster::Point)
			shader[1] += "#define LIGHT_POINT\n";
