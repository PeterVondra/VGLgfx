...
namespace vgl
{
	namespace vk
	{
		...
		Pipeline GraphicsContext::m_TextBoxPipeline;
		Pipeline GraphicsContext::m_Shape2DPipeline;
		Pipeline GraphicsContext::m_Circle2DPipeline;
		Pipeline GraphicsContext::m_ParticlePipeline;
		...
		void GraphicsContext::createSkyboxPipelines(RenderPass& p_RenderPass) {
			...
			auto layout = contextPtr->m_DescriptorSetLayoutCache.createDescriptorSetLayout({ {0} }, {}, { {1} });
			...
			layout = contextPtr->m_DescriptorSetLayoutCache.createDescriptorSetLayout({ {0} });
			pipelineInfo.p_DescriptorSetLayout = &layout;
		...
		CubeMap GraphicsContext::generateAtmosphericScatteringCubeMap(Vector2i p_Size, AtmosphericScatteringInfo& p_Info)
		{
			Context* contextPtr = &ContextSingleton::getInstance();
			...
			CommandBuffer cmd(contextPtr->m_DefaultCommandPool, Level::Primary);
		...
		CubeMap GraphicsContext::generateIrradianceMap(CubeMap& p_CubeMapImage, Vector2i p_Size)
		{
			Context* contextPtr = &ContextSingleton::getInstance();
		...
		CubeMap GraphicsContext::getCubeMapFromHDR(Image& p_HDRImage, Vector2i p_Size)
		{
			Context* contextPtr = &ContextSingleton::getInstance();
		...
		CubeMap GraphicsContext::generatePreFilteredMaps(CubeMap& p_CubeMapImage, Vector2i p_InitialSize)
		{
			Context* contextPtr = &ContextSingleton::getInstance();
