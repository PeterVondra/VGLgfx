...
void main()
{
    ...
    vec3 avgColor = textureLod(scene_image, vec2(0.5f, 0.5f), 1000.0).rgb + textureLod(scene_image, vec2(0.5f, 0.5f), 7.0).rgb/2;
    ...
    exposure = clamp(exposure, pushConstants.min_exposure, pushConstants.max_exposure);
    
    vec3 color = texture(scene_image, UV).rgb;
    vec3 result = vec3(1.0) - exp(-color * exposure);
	
    // also gamma correct while we're at it
    //outColor.rgb = pow(reinhardExtendedLuminance(result, 30.0f), vec3(1.0 / pushConstants.gamma));
    outColor.rgb = LinearTosRGB(reinhardExtendedLuminance(result, 5.0f));
    //outColor.rgb = LinearTosRGB(ACESFitted(result)*1.8f);
