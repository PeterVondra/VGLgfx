...
namespace vgl
{
	namespace Text
	{
		...
		bool FontLoader::getFontFromPath(const char* p_Path, Font& p_Font)
		{
			...
			{
				Utils::Logger::logMSG("Failed to initialize freetype\n", "Font", Utils::Severity::Error);
				return false;
			}

			if (FT_New_Face(ftlib, p_Path, 0, &p_Font.face))
			{
				Utils::Logger::logMSG("Failed to laod font file " + std::string(p_Path) + "\n", "Font", Utils::Severity::Error);
				return false;
			}
			...
			for (wchar_t c = 0; c < 128; c++)
			{
				...
				{
					Utils::Logger::logMSG("Failed to load character: " + (char)c, "Font", Utils::Severity::Error);
					continue;
				}
