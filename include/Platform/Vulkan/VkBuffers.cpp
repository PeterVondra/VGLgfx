...
namespace vgl
{
	namespace vk
	{
		...
		{
#ifdef USE_LOGGING
			Utils::Logger::logMSG("Destroyed Index buffer", "VkIndexBuffer", Utils::Severity::Trace);
#endif
		...
		void IndexBuffer::fill(std::vector<uint32_t>* p_BufferData)
		{
			if (!p_BufferData) {
#ifdef USE_LOGGING
				Utils::Logger::logMSG("Buffer data is nullptr", "VkIndexBuffer", Utils::Severity::Error);
#endif
				return;
			}
