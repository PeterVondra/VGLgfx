...
namespace vgl
{
	namespace vk
	{
		...
		bool RenderPass::create()
		{
			...
			{
				Utils::Logger::logMSG("Failed to create render pass, no attachments submitted", "RenderPass", Utils::Severity::Warning);
				m_IsValid = false;
				return false;
			}
			...
			{
#ifdef USE_LOGGING
				Utils::Logger::logMSG("Failed to create render pass", "VkRenderPass", Utils::Severity::Error);
#endif
				m_IsValid = false;
				return false;
			}
...
#ifdef USE_LOGGING
			Utils::Logger::logMSG("Succesfully created renderpass", "VkRenderPass", Utils::Severity::Trace);
#endif
			m_IsValid = true;
			return true;
		}
		...
		bool g_Pipeline::create(g_PipelineInfo p_PipelineInfo)
		{
			if (m_IsValid) {
#ifdef USE_LOGGING
				Utils::Logger::logMSG("Graphics Pipeline was already created, destroy it first!", "vk::g_Pipeline", Utils::Severity::Warning);
#endif
				return false;
			}
...
#ifdef USE_LOGGING
				Utils::Logger::logMSG("Failed to create pipeline layout", "Rendering", Utils::Severity::Error);
#endif
				return false;
			}
...
#ifdef USE_LOGGING
				Utils::Logger::logMSG("Failed to create graphics pipeline, p_RenderPass == nullptr", "Rendering", Utils::Severity::Error);
#endif
				return false;
			}
			else if (vkCreateGraphicsPipelines(m_ContextPtr->m_Device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_Pipeline) != VK_SUCCESS){
#ifdef USE_LOGGING
				Utils::Logger::logMSG("Failed to create graphics pipeline", "Rendering", Utils::Severity::Error);
#endif
				return false;
			}
#ifdef USE_LOGGING
			else
				Utils::Logger::logMSG("Succesfully created graphics pipeline", "Rendering", Utils::Severity::Trace);
#endif
			m_IsValid = true;
