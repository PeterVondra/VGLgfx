...
float FindBlockerDistance_DirectionalLight(vec4 shadowCoord, float uvLightSize)
{
	...
	for (int i = 0; i < BLOCKER_SEARCH; i++)
	{
		...
		if ( shadowCoord.z > -1.0 && shadowCoord.z < 1.0 ){
            float dist = texture( DIR_SHADOWMAP, vec2(shadowCoord.s, 1.0f - shadowCoord.t) + offset*searchWidth).r + max(0.00005 * (1.0 - dot(normal, ubo.directional_light.Direction.xyz)), 0.000005);
