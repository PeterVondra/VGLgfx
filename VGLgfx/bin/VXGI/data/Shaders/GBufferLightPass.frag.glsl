...
#define BLOCKER_SEARCH 64
#define PENUMBRA_ESTIMATE 3
#define lightSize 0.003
#define NEAR 1
#define EPSILON 0.0009

layout(binding = 1) uniform sampler2D in_Position;
layout(binding = 2) uniform sampler2D in_Normal;
layout(binding = 3) uniform sampler2D in_Albedo;
// R = Metallic, G = Roughness, B = Ambient Occlusion, A = Depth
layout(binding = 4) uniform sampler2D in_MRAA;
layout(binding = 5) uniform sampler2D in_SSAO;
...
float emissive;
float gSSAO;
float s_bias; // Shadow bias

float g_scattering = 21e-6;

float scattering(float light_dot_view)
{
    float result = 1.0f - g_scattering*g_scattering;
    result = (4.0f * PI * pow(pushConstants.mie_scatter_C + g_scattering*g_scattering - (2.0f * g_scattering) * light_dot_view, 1.5f));
    return result;
}
...
#ifdef SHADOW_MAPPING
float ditherPattern[16] = {0.0f, 0.5f, 0.125f, 0.625f, 0.75f, 0.22f, 0.875f, 0.375f, 0.1875f, 0.6875f, 0.0625f, 0.5625, 0.9375f, 0.4375f, 0.8125f, 0.3125};
float dither4x4(vec3 p_Color)
{
...
void main()
{
    gSSAO = texture(in_SSAO, UV).r;
    fragPos = texture(in_Position, UV);

    albedo_t = texture(in_Albedo, UV);
	albedo = albedo_t.rgb;

    albedo.r = max(0.01f, albedo.x);
    albedo.g = max(0.01f, albedo.y);
    albedo.b = max(0.01f, albedo.z);
	...
	vec4 normal_e = texture(in_Normal, UV);
    normal = normal_e.rgb;
    emissive = normal_e.a;
    ...
    if(emissive > 1.0f){
        outColor.rgb = albedo_t.rgb * emissive;
...
vec3 fresnelSchlick(float cosTheta, vec3 F0)
{
	...
	return max(F0 + (1.0f - F0) * pow(1.0f - min(cosTheta, 1.0), 5.0f), 0.0f);
}

vec3 fresnelSchlickRoughness(float cosTheta, vec3 F0, float roughness)
{
    cosTheta = min(cosTheta, 1.0);
    return max(F0 + (max(vec3(1.0 - roughness), F0) - F0) * pow(1.0 - cosTheta, 5.0), 0.0f);
}   


vec3 lightDirPBR(vec3 p_Dir, vec3 p_Color)
{
    if(length(max(p_Color, 0.0f)) <= 0.0f)
        return vec3(0.0f);
	...
	float dist = length(ubo.viewPos.xyz - fragPos.xyz);
	...
	vec3 ambient = (Kd * diffuse + specular) * (emissive > 1.0f ? emissive : ao * gSSAO * 0.025f);
	#else
	Ks = F;
	Kd = vec3(1.0f) - Ks;
	Kd *= 1.0f - metallic;
	vec3 ambient = normalize(lightColor) * albedo * (emissive > 1.0f ? emissive : ao * gSSAO * 0.025f);
	#endif

	
	#ifdef SHADOW_MAPPING
    vec2 shadow = PCSS_DirectionalLight();
...
vec3 lightPointPBR(PointLight p_PointLight)
{
    if(length(max(p_PointLight.Color.rgb, 0.0f)) <= 0.0f)
        return vec3(0.0f);
	vec3 lightColor = max(p_PointLight.Color.rgb, 0.0001f);
...
float textureProj(vec4 shadowCoord, vec2 off)
{
	if(shadowCoord.z > -1.0 && shadowCoord.z < 1.0 ){
        float sz = texture( DIR_SHADOWMAP, vec2(shadowCoord.s, 1.0f - shadowCoord.t) + off ).r;
		if (shadowCoord.w > 0.0f && sz + s_bias < shadowCoord.z ) 
			return 0.0f;
        }
	return 1.0f;
}
...
float FindBlockerDistance_DirectionalLight(vec4 shadowCoord, float uvLightSize)
{
	...
	for (int i = 0; i < BLOCKER_SEARCH; i++)
	{
		...
		if ( shadowCoord.z > -1.0 && shadowCoord.z < 1.0 ){
            float dist = texture( DIR_SHADOWMAP, vec2(shadowCoord.s, 1.0f - shadowCoord.t) + offset*searchWidth).r + s_bias;
...
vec3 volumetric_light()
{
    ...
    vec3 step_ = ray_dir * step_length;
    
    vec3 current_pos = ubo.viewPos.xyz;

    vec3 accum_fog = vec3(0.0f);
    if(length(max(ubo.directional_light.Color.xyz, 0.0f)) <= 0.0f)
        return vec3(0.0f);
    vec3 color = normalize(ubo.directional_light.Color.xyz);
    for(int i = 0; i < NB_STEPS; i++){
        vec4 worldInShadowView = bias*ubo.lightSpaceMat*vec4(current_pos, 1.0f);
        worldInShadowView /= worldInShadowView.w;

        float shadow_map_c = texture(DIR_SHADOWMAP, vec2(worldInShadowView.s, 1.0f - worldInShadowView.t)).r;

        if(shadow_map_c > worldInShadowView.z)
            accum_fog += scattering(dot(ray_dir, ubo.directional_light.Direction.xyz))*color;
        current_pos += step_;// * dither4x4(color);
    }
