	...
	{ 
		...
		void Descriptor::create(DescriptorSetInfo& p_Info)
		{
			...
			if (!p_Info.isValid()) {
				...
				VGL_LOG_MSG("In function Descriptor::create(...): DescriptorSetInfo is not valid", "Descriptor", Utils::Severity::Error);
#endif
				return;
			}
		...
		void Descriptor::destroy()
		{
			...
			if (m_IsValid) {
				...
				if (m_DescriptorSetInfo->p_VertexUniformBuffer.isValid()) {
					...
					VGL_LOG_MSG("Destroyed vertex stage uniform buffer\n", "Descriptor", Utils::Severity::Trace);
#endif
				}
				if (m_DescriptorSetInfo->p_FragmentUniformBuffer.isValid()) {
					m_DescriptorSetInfo->p_VertexUniformBuffer.destroy();
			...
			for (int32_t j = 0; j < m_ContextPtr->m_SwapchainImageCount; j++){
				...
				for (int32_t i = 0; i < 3; i++){
					if (m_DescriptorSetInfo->p_UniformBuffers[i].isValid()) {
						...
						VGL_LOG_MSG("Updated descriptor sets: uniform buffer[" + std::to_string(i) + "]", "Descriptor", Utils::Severity::Debug);
					}
				}
				if (m_DescriptorSetInfo->p_StorageBuffer.isValid()) {
					VkDescriptorBufferInfo bufferInfo = {};
					...
					vkUpdateDescriptorSets(m_ContextPtr->m_Device, 1, &dscWrite, 0, nullptr);
					VGL_LOG_MSG("Updated descriptor sets: storage buffer", "Descriptor", Utils::Severity::Debug);
				}
				...
				for (int32_t i = 0; i < m_ImageDescriptorInfo[j].size(); i++){
					...
					VGL_LOG_MSG("Updated descriptor sets: image descriptor[" + std::to_string(j) + "][" + std::to_string(i) + "]", "Descriptor", Utils::Severity::Debug);
				}
				for(int32_t i = 0; i < m_DescriptorSetInfo->p_ImageArrayDescriptors.size(); i++){
					...
					vkUpdateDescriptorSets(m_ContextPtr->m_Device, 1, &imageDscWrite, 0, nullptr);
					VGL_LOG_MSG("Updated descriptor sets: image array descriptor[" + std::to_string(j) + "][" + std::to_string(i) + "]", "Descriptor", Utils::Severity::Debug);
				}
			}
		}
