...
namespace vgl
{
	namespace vk
	{
		...
		template<> struct StorageImageDescriptorData<Image3D>
		{
			// p_ImageViews is for each mip level
			StorageImageDescriptorData(Image3D* p_Image_, uint32_t p_Binding_);
			StorageImageDescriptorData(Image3D* p_Image_, uint32_t p_Binding_, std::vector<VkImageView>* p_ImageViews) {}

			Image3D* p_Image = nullptr;
			uint32_t p_Binding = UINT32_MAX; // Has to be set
