...
namespace vgl
{
	namespace vk
	{
		...
		void PShadowMap::create(Vector2i p_Resolution, ImageFormat p_ImageFormat)
		{
			...
			m_TransformDescriptorInfo.p_VertexUniformBuffer = UniformBuffer(sizeof(Matrix4f) * 6, 0);
			m_TransformDescriptor.create(m_TransformDescriptorInfo);
		...
		void Renderer::submit(MeshData& p_MeshData, Transform3D& p_Transform, Image3D& p_VoxelImage)
		{
		...
		void Renderer::recordMeshDataSMD(MeshData& p_MeshData, Transform3D& p_Transform)
		{
			...
			for (int j = 0; j < p_MeshData.m_Materials.size(); j++) {
				...
				//}
				if (p_MeshData.m_Materials[j].m_AlbedoMap.isValid()) {
					if (p_MeshData.m_Materials[j].config.render){
...
#define CPSMPTR m_CurrentPShadowMapPtr
				...
				CPSMPTR->m_TransformDescriptor.copy(ShaderStage::VertexBit, CPSMPTR->m_View, sizeof(Matrix4f) * 6, 0);
#undef CPSMPTR
			}

			PSMT data = {};
			data.model = p_Transform.model;
			data.position = *m_CurrentPShadowMapPtr->m_Position;
				...
				//if (!(p_MeshData.m_Materials[i].m_PrevConfig == p_MeshData.m_Materials[i].config)) {
					...
					p_MeshData.m_MTLDescriptors[i].copy(ShaderStage::FragmentBit,
						Vector4f(
							...
							p_MeshData.m_Materials[i].config.m_Emissive
						), 4 * sizeof(float)
					);
