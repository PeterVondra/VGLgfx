...
public:
	...
	void onImGuiUpdate() {
		...
		if (ImGui::BeginViewportSideBar("##MainMenuBar", ImGui::GetMainViewport(), ImGuiDir_::ImGuiDir_Up, ImGui::GetFrameHeight(), ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_MenuBar)) {
			if (ImGui::BeginMenuBar()) {
				if (ImGui::BeginMenu("File")) {
					if (ImGui::MenuItem("Open")) {
						ImGuiFileDialog::Instance()->OpenDialog("Text", "Script File", "{.txt,.cpp,.h,.glsl,.vgls}", ".");
					}
					if (ImGui::MenuItem("Open Scene")) {
						//ImGuiFileDialog::Instance()->OpenModal("SceneOPEN", "Scene File(vgls)", "{.vgls}", ".");
					}
					...
					if (ImGui::MenuItem("Save Scene as...")) {
						ImGuiFileDialog::Instance()->OpenDialog("SceneSAVE", "Directory", "", ".");
					}
					if (ImGui::BeginMenu("Import")) {
						if (ImGui::MenuItem("Wavefront(.OBJ)")) {
							ImGuiFileDialog::Instance()->OpenDialog("IWOBJ", "Choose .OBJ File", ".obj", ".", 1, nullptr, ImGuiFileDialogFlags_Modal);
						}
						if (ImGui::MenuItem("Wavefront(.OBJ) - Flipped UV")) {
							ImGuiFileDialog::Instance()->OpenDialog("IWOBJFUV", "Choose .OBJ File", ".obj", ".", 1, nullptr, ImGuiFileDialogFlags_Modal);
						}
						ImGui::EndMenu();
					}
		...
		if (ImGuiFileDialog::Instance()->Display("IWOBJ")){
		...
		if (ImGuiFileDialog::Instance()->Display("IWOBJFUV")){
			if (ImGuiFileDialog::Instance()->IsOk()){
				vgl::Mesh3DComponent* model = new vgl::Mesh3DComponent();
				model->mesh = new vgl::MeshData();
				vgl::EntityNameComponent* name = new vgl::EntityNameComponent();
				...
				std::string fileName = ImGuiFileDialog::Instance()->GetCurrentFileName();
				std::string filePath = ImGuiFileDialog::Instance()->GetCurrentPath() + "/";
				model->mesh->m_FileName = fileName;

				vgl::OBJ_Loader::loadModel(filePath.c_str(), fileName.c_str(), model->mesh, true);
				name->Name = stripExtension(model->mesh->m_FileName, ".obj").c_str();
		...
		if (ImGui::BeginViewportSideBar("##MainStatusBar", ImGui::GetMainViewport(), ImGuiDir_::ImGuiDir_Down, ImGui::GetFrameHeight(), ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_MenuBar)) {
			if (ImGui::BeginMenuBar()) {
		...
		if (ImGui::TreeNodeEx("HDR##S", vgl::EditorColorScheme::TreeNodeFlags)) {
			...
			ImGui::Spacing();
			ImGui::Text("Max Exposure");
			ImGui::Spacing();
			ImGui::Text("Min Exposure");
			...
			ImGui::DragFloat("##312", &m_RenderPipeline.m_HDRInfo.max_exposure, 0.01);
			ImGui::DragFloat("##442", &m_RenderPipeline.m_HDRInfo.min_exposure, 0.01);
#undef max
#undef min
		...
		ImGui::Separator();
		if (ImGui::TreeNodeEx("Bloom##S", vgl::EditorColorScheme::TreeNodeFlags)) {
			ImGui::BeginColumns("##Bloom", 2);
			ImGui::Spacing();
			...
			ImGui::Spacing();
			ImGui::Text("Bloom Contribution");
			ImGui::Spacing();
			float offset = ImGui::GetColumnOffset();
			ImGui::NextColumn();
			ImGui::SetColumnWidth(1, ImGui::GetWindowSize().x - offset);
			ImGui::DragFloat("##43", &m_RenderPipeline.m_BloomThreshold, 0.001, 0);
			ImGui::DragFloat("##44", &m_RenderPipeline.m_FilterRadius, 0.0001, 0.000);
			ImGui::DragFloat("##45", &m_RenderPipeline.m_BloomMod.intensity, 0.01, 0.001);
			ImGui::DragFloat("##46", &m_RenderPipeline.m_BloomMod.contribution, 0.001, 0.0f, 1.0f);
			ImGui::EndColumns();
			vgl::DrawVec3Control("Knee", m_RenderPipeline.m_BloomKnee, 1.0f, 0.001f);
			ImGui::TreePop();
		}
		ImGui::Separator();
	...
	void sceneDisplay()
	{
		...
		if (ImGui::BeginPopupContextWindow("CNE")) {
			...
			if (ImGui::MenuItem(("Create New PointLight##" + std::to_string(C0unt)).c_str())) {
				vgl::EntityNameComponent* name = new vgl::EntityNameComponent();
				...
				C0unt++;
				m_Scene.addEntity(*name, *point_light);
			}
			ImGui::EndPopup();
		}
			...
			if (open) {
				...
				if (point_light) {
					...
					if (ImGui::TreeNodeEx(name->Name.c_str(), vgl::EditorColorScheme::TreeNodeFlags)) {
						...
						Matrix4f model = point_light->Transform.model;
						Vector3f matrixTranslation, matrixRotation, matrixScale;
						ImGuizmo::DecomposeMatrixToComponents(&model[0], matrixTranslation.get(), matrixRotation.get(), matrixScale.get());
						vgl::DrawVec3Control("Translation", matrixTranslation);
						ImGui::DragFloat("Radius", &point_light->Radius, 0.1f);

						ImGuizmo::RecomposeMatrixFromComponents(matrixTranslation.get(), matrixRotation.get(), matrixScale.get(), &model[0]);

						ImGuiIO& io = ImGui::GetIO();
						ImGuizmo::SetRect(m_ViewportData.m_Position.x, m_ViewportData.m_Position.y, m_ViewportData.m_Size.x, m_ViewportData.m_Size.y);
						ImGuizmo::Manipulate(&m_Camera.getViewMatrix()[0], &m_Camera.getPerspectiveMatrix()[0],
							mCurrentGizmoOperation, mCurrentGizmoMode, &model[0], NULL, NULL);
						ImGuizmo::DecomposeMatrixToComponents(&model[0], matrixTranslation.get(), matrixRotation.get(), matrixScale.get());

						point_light->Transform.setPosition(matrixTranslation);
					...
					for (int j = 0; j < mesh->mesh->m_SubMeshIndices.size(); j++) {
						...
						if (ImGui::TreeNodeEx(name.c_str(), vgl::EditorColorScheme::TreeNodeFlags)) {
							...
							if (ImGui::TreeNodeEx((name + "##" + std::to_string(i)).c_str(), vgl::EditorColorScheme::TreeNodeFlagsFramed)) {
								...
								ImGui::DragFloat("Emissive", &mesh->mesh->getMaterial(j).config.m_Emissive, 0.01, 0.0f);
