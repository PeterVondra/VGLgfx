...
<<<<<<< HEAD
				m_CurrentWindow = p_CurrentWindow;

				io.Fonts->AddFontFromFileTTF("data/Fonts/arial.ttf", 14.0f, NULL, io.Fonts->GetGlyphRangesDefault());
				ImGui_ImplGlfw_InitForVulkan(p_CurrentWindow->getGLFWWindow(), true);
				...
				info.width = m_CurrentWindow->getWindowSize().x;
				info.height = m_CurrentWindow->getWindowSize().y;
				info.layers = 1;

				m_Framebuffers.resize(m_ContextPtr->m_SwapchainImageCount);

				for (uint32_t i = 0; i < m_ContextPtr->m_SwapchainImageCount; i++) {
					attachment[0] = m_CurrentWindow->m_Swapchain.m_SwapchainImageViews[i];
...
>>>>>>> parent of e6feae2 (Rendering available)
				...
				inline void init(RenderPass& p_RenderPass)
				{
					...
					if (vkCreateDescriptorPool(m_ContextPtr->device, &poolInfo, nullptr, &descriptorPool) != VK_SUCCESS)
					...
					init_info.Instance = m_ContextPtr->instance;
					init_info.PhysicalDevice = m_ContextPtr->mainPhysicalDevice.physicalDevice;
					init_info.Device = m_ContextPtr->device;
					init_info.QueueFamily = m_ContextPtr->mainPhysicalDevice.queueFamily.graphicsFamily;
					init_info.Queue = m_ContextPtr->graphicsQueue;
		...
		public:
			...
			inline void init(RenderPass& p_RenderPass)
			{
				...
				_ImguiLayout = { {
						{ exz::ShaderDataType::ImVec2f, 0, "inPosition" },
						{ exz::ShaderDataType::ImVec2f, 1, "inUv" },
						{ exz::ShaderDataType::ImVec4f, 2, "inColor" }
					},0
				};
